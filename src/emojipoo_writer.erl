...
serialize_state(#state{index_file = File, 
                       index_file_pos = Position} = State) ->
...
handle_cast({add, Key, {?TOMBSTONE, TStamp}}, State) when is_binary(Key) ->
   NewState =
     case emojipoo_util:has_expired(TStamp) of
        true -> State; % don`t write if expired
        false ->
...
handle_cast({add, Key, ?TOMBSTONE}, State) when is_binary(Key) ->
   ...
   {noreply, NewState};
handle_cast({add, Key, {Value, TStamp}}, State) when is_binary(Key), 
                                                     is_binary(Value) ->
   NewState =
     case emojipoo_util:has_expired(TStamp) of
        true -> State; % don`t write if expired
...
archive_nodes(#state{nodes = [#node{level = N, 
                                    members = [{_,{Pos,_Len}}]}], 
                     last_node_pos = Pos} = State) when N > 0 ->
...
append_node(Level, Key, Value, 
            #state{nodes = [#node{level = Level, 
                                  members = MembersList, 
                                  size = NodeSize} = CurrNode | RestNodes ], 
                   value_count = VC, 
                   tombstone_count = TC} = State) ->
   ...
   State2 = State#state{nodes = [CurrNode#node{members = NodeMembers, 
                                               size = NewSize} | RestNodes],
                        value_count = VC1, 
                        tombstone_count = TC1},
...
flush_node_buffer(#state{nodes=[#node{level = Level, 
                                      members = NodeMembers}|RestNodes], 
                         compress = Compress, 
                         index_file_pos = NodePos} = State) ->
