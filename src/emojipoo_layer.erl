...
-record(state,
    % index records for each version
    {
        a,
        b,
        c,
        % next deeper state machine pid in the pile
        next,
        % directory where the files go
        dir,
        % this layer`s depth from top of pile
        depth,
        % ref to wait for after injecting to next level
        inject_done_ref,
        % merging process id
        merge_pid,
        % merge_pid monitor
        ...
        % pid of process that started the layer
        parent
    }
).
...
range(LayerPid, SendTo, Range, List, FilterMap) ->
    {ok, Folders} = gen_statem:call(
        LayerPid, {init_range_fold, SendTo, Range, List, FilterMap}, 60000
    ),
    Folders.
...
terminate(Reason, _StateName, _StateData) when
    Reason == normal;
    Reason == shutdown
->
    %% XXX close all here
...
%% empty lookup
handle_event({call, From}, {lookup, _}, ready, #state{
    a = undefined,
    b = undefined,
    c = undefined,
    next = undefined
...
handle_event(cast, {merge_done, 0, MergeFileName}, StateName, State) when
    StateName == merging;
    % happens on startup
    StateName == ready
->
...
%% Merge fits here
handle_event(cast, {merge_done, Count, OutFileName}, StateName, State) when
    Count =< ?BTREE_SIZE(State#state.depth), StateName == merging;
...
handle_event(cast, {merge_done, _, OutFileName}, StateName, State) when
    State#state.next =:= undefined, StateName == merging;
...
handle_event(cast, {ok, MonRef}, wait_inject, State) when
    MonRef == State#state.inject_done_ref
->
...
handle_event(info, {'DOWN', MonRef, _, _, Reason}, wait_inject, State) when
    MonRef == State#state.inject_done_ref
->
    exit(Reason);
...
handle_event({call, _}, {inject, _, _}, merging, _) ->
    {keep_state_and_data, [postpone]};
handle_event({call, _}, {inject, _, _}, StateName, State) when
...
->
    ...
    {keep_state_and_data, [postpone]};
handle_event({call, From}, {inject, MonRef, FileName}, StateName, State) when
    StateName == ready, State#state.c == undefined;
    StateName == wait_inject, State#state.c == undefined
->
    %{merge, From}
...
handle_event(
    {call, From},
    {init_range_fold, SendTo, Range, List, FilterMap},
    ready,
    ...
    case Next of
        ...
        _ ->
            ...
            {keep_state, State#state{folding = FoldingPIDs}, {reply, From, Reply}}
    end;
...
handle_event(info, {'EXIT', Pid, Reason}, _, State) when
    Pid == State#state.merge_pid
->
    ?log("*** merge_died: ~p~n", [Reason]),
    %% XXX BAD
    State1 = restart_merge_then_loop(State#state{merge_pid = undefined}, Reason),
    {keep_state, State1};
handle_event(info, {'EXIT', Pid, _}, _, State) when
    Pid == hd(State#state.folding);
...
link_filename(Prefix, Suffix, #state{
    dir = Dir,
    depth = Depth
}) ->
    filename:join(
        Dir,
        Prefix ++ "-" ++ integer_to_list(Depth) ++
            "-" ++ Suffix ++ ".data"
    ).
