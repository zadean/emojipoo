...
% top of the pile, first in chain of layers
-record(state, {
    top :: pid() | undefined,
    % transaction log
    log :: #log{} | undefined,
...
-spec start_link(Dir :: string(), Opts :: [config_option()]) ->
    {ok, server()} | ignore | {error, term()}.
start_link(Dir, Opts) ->
...
open_layers(Dir, Options) ->
    ...
    Fold = fun(FileName, {LastMinDepth, LastMaxDepth}) ->
        case layer_from_filename(FileName) of
            {ok, Depth} ->
                {erlang:min(LastMinDepth, Depth), erlang:max(LastMaxDepth, Depth)};
            _ ->
...
handle_cast({bottom_level, N}, #state{log = Log, top = Top} = State) when
    N > State#state.max_depth
->
    State2 = State#state{
        max_depth = N,
...
handle_call(
    {range, FoldWorkerPID, Range, FilterMap},
    _From,
    ...
    {reply, Result, State};
handle_call({put, Key, Value, Expiry}, _From, State) when
    is_binary(Key),
    is_binary(Value)
->
...
do_put(Key, Value, Expiry, #state{log = Log, top = Top} = State) when
    Log =/= undefined
->
