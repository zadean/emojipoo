...
recover(Directory, TopLevel, MinLevel, MaxLevel, Config) when
    MinLevel =< MaxLevel,
    is_integer(MinLevel),
    is_integer(MaxLevel)
->
...
fill_cache({Key, Value}, Cache) when
    is_binary(Value);
    Value =:= ?TOMBSTONE
->
    enter(Key, Value, Cache);
fill_cache({Key, {Value, _TStamp} = Entry}, Cache) when
    is_binary(Value);
    Value =:= ?TOMBSTONE
->
...
cache_from_binary(LogBinary, Directory) ->
    case emojipoo_util:decode_crc_data(LogBinary, [], []) of
        ...
        {partial, KVs, _ErrorData} ->
            ?error(
                "ignoring undecypherable bytes in ~p~n",
                [?LOGFILENAME(Directory)]
            ),
            fill_cache(KVs, empty())
    end.
...
-spec do_add(#log{}, binary(), binary() | ?TOMBSTONE, non_neg_integer() | infinity, pid()) ->
    {ok, #log{}} | {full, #log{}}.
    ...
    LastSync =
        case application:get_env(?APP, sync_strategy) of
            ...
            {ok, {seconds, N}} ->
                ...
                if
                    (MicrosSinceLastSync div 1000000) >= N ->
                        file:datasync(File),
                        ?NOW;
                    true ->
                        Log#log.last_sync
...
batch1(
    ...
    Expiry =
        case emojipoo_util:get_opt(expiry_secs, Config) of
            0 -> infinity;
            DatabaseExpiryTime -> emojipoo_util:expiry_time(DatabaseExpiryTime)
        end,
    ...
    EntriesFun =
        fun
            ({put, Key, Value}) when Expiry == infinity ->
                {Key, Value};
            ({put, Key, Value}) ->
                {Key, {Value, Expiry}};
