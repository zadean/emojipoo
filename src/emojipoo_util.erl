...
-define(SNAPPY_COMPRESSION, 1). %% remove? add back later?
-define(GZIP_COMPRESSION,   2).
-define(LZ4_COMPRESSION,    3). %% remove? add back later?
...
compress(none, Bin) -> {?NO_COMPRESSION, Bin};
compress(snappy, Bin) ->
...
decode_index_node(Level, Data) ->
   ...
   case decode_kv_list(TermData) of
      {ok, KVList} -> % erlang term as binary, unused...
...
crc_encapsulate_kv_entry(Key, {Value, TStamp}) when is_binary(Value) ->
   crc_encapsulate( [?TAG_KV_DATA2, 
                     <<TStamp:32, (byte_size(Key)):32/unsigned>>, Key, Value]);
crc_encapsulate_kv_entry(Key, Value) when is_binary(Value) ->
   crc_encapsulate( [?TAG_KV_DATA, 
                     <<(byte_size(Key)):32/unsigned>>, Key, Value] );
crc_encapsulate_kv_entry(Key, {Pos,Len}) when Len < 16#ffffffff ->
   crc_encapsulate( [?TAG_POSLEN32, 
                     <<Pos:64/unsigned, Len:32/unsigned>>, Key] ).
...
-spec decode_kv_list( binary() ) -> {ok, [ kventry() ]}  | 
...
decode_kv_list(<<?BISECT_ENCODED, CRC:32/unsigned, Binary/binary>>) ->
   ...
   if CRC == CRCTest ->
         {bisect, Binary};
      true ->
         {bisect, emojipoo_vbisect:from_orddict([])} % corrupt so empty
   end.
...
decode_crc_data(<< BinSize:32/unsigned, CRC:32/unsigned, Bin:BinSize/binary, 
                   ?TAG_END, Rest/binary >>, Broken, Acc) ->
...
decode_kv_data(<<?TAG_KV_DATA, 
                 KLen:32/unsigned, Key:KLen/binary, Value/binary >>) ->
   {Key, Value};
decode_kv_data(<<?TAG_DELETED, 
                 Key/binary>>) ->
   {Key, ?TOMBSTONE};
decode_kv_data(<<?TAG_KV_DATA2, 
                 TStamp:32/unsigned, KLen:32/unsigned, 
                 Key:KLen/binary, Value/binary >>) ->
   {Key, {Value, TStamp}};
decode_kv_data(<<?TAG_DELETED2, 
                 TStamp:32/unsigned, Key/binary>>) ->
   {Key, {?TOMBSTONE, TStamp}};
decode_kv_data(<<?TAG_POSLEN32, 
                 Pos:64/unsigned, Len:32/unsigned, Key/binary>>) ->
   {Key, {Pos,Len}};
decode_kv_data(<<?TAG_BATCH, 
                 Rest/binary>>) ->
