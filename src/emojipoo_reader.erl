...
open(Name, Config) ->
    case proplists:get_bool(sequential, Config) of
        true ->
            ReadBufferSize = emojipoo_util:get_opt(
                read_buffer_size,
                Config,
                512 * 1024
            ),
...
fold1(File, Fun, Acc0) ->
    case next_leaf_node(File) of
        eof -> Acc0;
        {ok, Node} -> fold0(File, Fun, Node, Acc0)
    end.
...
range_fold_from_here_stop_fun(Fun, Range) ->
    fun
        ({Key, _}, Acc) when not ?KEY_IN_TO_RANGE(Key, Range) ->
            {stop, {done, Acc}};
        ({Key, Value}, Acc) when ?KEY_IN_FROM_RANGE(Key, Range) ->
            case emojipoo_util:is_expired(Value) of
                true -> {continue, Acc};
                false -> {continue, Fun(Key, emojipoo_util:get_value(Value), Acc)}
            end;
        (_Huh, Acc) ->
            {continue, Acc}
    end.
    ...
    fun
        ...
        ({Key, {?TOMBSTONE, TStamp}}, {N1, Acc}) when ?KEY_IN_FROM_RANGE(Key, Range) ->
            case emojipoo_util:has_expired(TStamp) of
                ...
                false -> {continue, {N1, Fun(Key, ?TOMBSTONE, Acc)}}
            end;
        ({Key, Value}, {N1, Acc}) when ?KEY_IN_FROM_RANGE(Key, Range) ->
            case emojipoo_util:is_expired(Value) of
                true -> {continue, {N1, Acc}};
                false -> {continue, {N1 - 1, Fun(Key, emojipoo_util:get_value(Value), Acc)}}
            end;
        (_, Acc) ->
            {continue, Acc}
    end.
    ...
    case next_leaf_node(File) of
        ...
        {ok, #node{members = Members}} ->
            ...
            case fold_until_stop(Fx, Acc0, Members) of
                ...
                {ok, Acc1} -> range_fold_from_here(Fun, Acc1, File, Range, undefined)
            end
    end;
    ...
    case next_leaf_node(File) of
        ...
        {ok, #node{members = Members}} ->
            ...
            case fold_until_stop(Fx, {N0, Acc0}, Members) of
                {stopped, Result} -> Result;
                {ok, {N2, Acc1}} -> range_fold_from_here(Fun, Acc1, File, Range, N2)
            end
    end.
